...
describe("HelloWorld", function () {
  ...
  it("Should execute transferOwnership correctly", async function () {
    ...
    //now i wait for transaction to be included in the blockchain
    await tx.wait();
    const contractOwner = await helloWorldContract.owner();
    // https://www.chaijs.com/api/bdd/#method_equal
  ...
  it("Should not allow anyone other than owner to change text", async function () {
    const { helloWorldContract, accounts } = await loadFixture(deployContract);
    await expect(
      helloWorldContract.connect(accounts[1]).setText("I don't have permission")
    ).to.be.revertedWith("Caller is not the owner");
  });

  it("Should change text correctly", async function () {
    const { helloWorldContract } = await loadFixture(deployContract);
    const newText = "Ethereum is the best";
    const tx = await helloWorldContract.setText(newText);
    await tx.wait();
    const helloWorldText = await helloWorldContract.helloWorld();
    expect(helloWorldText).to.equal("Ethereum is the best");
  });
});
